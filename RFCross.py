...
params = {'n_estimators': 2000, 'max_depth': 1, 'max_leaf_nodes': 3 }
cv= KFold(n_splits=10, shuffle=True)
for i, (train, test) in enumerate(cv.split(X_train, y_train)):
        model = RandomForestClassifier(**params).fit(X_train.iloc[train], y_train.iloc[train])
...
df2.to_csv("D:\\project\\sahuanlan\\predtrainRF10.csv", header=True, index=None)
